...
public:
    ...
    size_t move_balloon(mmu::jvm_balloon_vma *vma, unsigned char *dest, unsigned char *src);
private:
...
size_t balloon::move_balloon(mmu::jvm_balloon_vma *vma, unsigned char *dest, unsigned char *src)
{
...
void jvm_balloon_fault(balloon *b, exception_frame *ef, mmu::jvm_balloon_vma *vma)
{
    ...
    WITH_LOCK(balloons_lock) {
        ...
        decode->memcpy_fixup(ef, b->move_balloon(vma, dest, src));
    }
}
