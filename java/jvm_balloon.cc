...
class balloon {
public:
    explicit balloon(unsigned char *jvm_addr, jobject jref);
...
{
    _jvm_end_addr = _jvm_addr + balloon_size;
    _addr = align_up(_jvm_addr, mmu::huge_page_size);

    end = align_down(_jvm_end_addr, mmu::huge_page_size);
    start = _addr;
...
balloon::balloon(unsigned char *jvm_addr, jobject jref)
    : _jvm_addr(jvm_addr), _jref(jref)
{
...
size_t balloon::move_balloon(unsigned char *dest, unsigned char *src)
{
    ...
    // carre of arrange things for us.
    mmu::map_anon(_addr, _hole_size, flags, perms);
    empty_area();
    return end - dest;
}

size_t jvm_balloon_shrinker::jvm_balloon_expand(JavaVM *vm, size_t size)
{
    JNIEnv *env = NULL;
    size_t ret = 0;

    // Since we are expected to be called from the shrinking code in OSv, we
    // are not a java thread. We attach to one. That has to be undone before
    // we leave.
...
size_t jvm_balloon_shrinker::jvm_balloon_shrink(JavaVM *vm, size_t size)
{
    JNIEnv *env = NULL;
    if (vm->AttachCurrentThread((void **) &env, NULL) != 0) {
        assert(0);
...
size_t jvm_balloon_shrinker::request_memory(size_t s)
{
    return jvm_balloon_expand(_vm, s);
}

size_t jvm_balloon_shrinker::release_memory(size_t s)
{
    return jvm_balloon_shrink(_vm, s);
}
