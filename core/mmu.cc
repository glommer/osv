...
// Balloon is backed by no pages, but in the case of partial copy, we may have to back some
// of the pages. For that and for that only, we initialize a page allocator.
...
void jvm_balloon_vma::fault(uintptr_t fault_addr, exception_frame *ef)
{
    ...
    // FIXME : Try to map the original worker with a huge page, and try to see
    // if we succeed. Using a huge page is harder than it seems, because the
    // JVM is not guaranteed to copy objects in huge page increments - and it
    // usually won't.  If we go ahead and map a huge page subsequent copies
    // *from* this location will not fault and we will lose track of the
    // partial copy count.
